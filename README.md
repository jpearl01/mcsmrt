...
### Tutorial:
This section is a walk through for how to use MCSMRT using sample data from BEI sequencing on PacBio. Before this, CCS and demultiplexing were run on the samples using the Reads of Insert (ROI) protocol from SMRT link. A filtering criteria of minimum number of passes = 5 and predicted accuracy = 90 was used. Successful completion of the ROI protocol creates a results directory in the location as configured by your lab tech. A directory similar to the one produced by ROI is in the tutorial folder which comes along with cloning the MCSMRT repository. You can then run each of these steps to learn how to use MCSMRT.  
