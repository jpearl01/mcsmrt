...
def map_to_human_genome (base_name, human_db)                                                                                                               
  #align all reads to the human genome                                                                                                                   
  `bwa mem -t 15 #{human_db} corrected.fq > corrected.sam`
...
def process_each_file (samps, log, all_bc_reads, table, table_2, ccs_hash, human_db, script_directory)
  samps.each do |rec|
...
#        puts non_filt.inspect
      ...
      if File.exist?("corrected_final.fastq")
        if File.zero?("corrected_final.fastq")
          ...
          `ruby #{script_directory}/parse_primer_matching_output_final.rb -f corrected_final.fastq -p #{base_name}_primer_map.txt -o #{base_name}_oriented_trimmed.fastq -s #{base_name}_singletons.fastq -m #{base_name}_singletons_primer_map.txt`
      ...
      if File.exist?("#{base_name}_singletons.fastq")
        if File.zero?("#{base_name}_singletons.fastq")
          ...
          `ruby #{script_directory}/retrieve_singletons.rb -f #{base_name}_singletons.fastq -p #{base_name}_singletons_primer_map.txt -w #{base_name}_singletons_forward.fastq -r #{base_name}_singletons_reverse.fastq -x #{base_name}_singletons_primer_map_forward_trimmed.fastq -s #{base_name}_singletons_primer_map_reverse_trimmed.fastq`
...
def run_usearch_and_other (ee, uchime_db_file, utax_db_file, lineage_fasta_file, script_directory) 
  ...
  `sh #{script_directory}/uparse_commands.sh all_ee#{ee} #{ee} #{uchime_db_file} #{utax_db_file}` 
  
  # Running the command to give a report of counts
  `ruby #{script_directory}/get_report.rb all_ee#{ee}`
  ...
  `ruby #{script_directory}/map_blast_with_otus.rb -u all_ee#{ee}_table_utax_map.txt -b all_ee#{ee}_blast.txt -o all_ee#{ee}_otu_blast_formatted.txt`
  
  # Running the script which dismantles the taxonomic assignments field
  `ruby #{script_directory}/parse_final_header.rb -h all_ee#{ee}_otu_blast_formatted.txt -o all_ee#{ee}_otu_blast_formatted2.txt`  
end
...
pb_projects.each do |id, samps|
  ...
  #puts ccs_hash
  process_each_file(samps, log, all_bc_reads, table, table_2, ccs_hash, human_db, script_directory)
end
 
# Calling the method which runs usearch and processes its output
run_usearch_and_other(ee, uchime_db_file, utax_db_file, lineage_fasta_file, script_directory)
