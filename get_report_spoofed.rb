...
opts = Trollop::options do
	opt :sample, "sample name (all_ee1 in our case, when ee = 1)", :type => :string, :short => "-s"
end 

##### Assigning variables to the input and creating output files
sample = opts[:sample]                                                                   
table_utax_map_file = File.open(sample+"_spoofed_table_utax_map.txt", 'w')
report_file = File.open(sample+"_spoofed_report.txt", 'w')

##### Class for storing the counts at each step
...
utax_table_file = File.open("#{sample}_spoofed_reads.utax", "r")
utax_table_hash = {}
